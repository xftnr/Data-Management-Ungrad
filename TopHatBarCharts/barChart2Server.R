...
# Build and Output Throughput Plot ------------------------
  output$Density__2_ <- renderPlotly({
    plot = dfA1() %>% dplyr::mutate(sum_sales_percent=percent_rank(sum_sales)) %>% ggplot() + geom_bar(mapping = aes(x=category, y=sum_sales_percent, colour=region, fill=region), stat = "identity") +
    ...
    if( ! is.na(input$plotWidth__2_) & ! is.na(input$plotHeight__2_))
     ggplotly(plot, tooltip = c("sales"), session="knitr", width = input$plotWidth__2_, height = input$plotHeight__2_)
    else
