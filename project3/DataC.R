dfC <- eventReactive(c(input$selectworkclass_1), { 
  project <- "https://data.world/xftnr/s18-db-project-3-dataset" 
  data.world::set_config(cfg_env("DW_API")) 
  paramQuery <- data.world::qry_sql(
    "   
    select workclass, avg(hours_per_week) as hours_per_week, education
    from adult_new
    where workclass in (?, ?, ?, ?, ?, ?, ?, ?)
...
dfC1 <- eventReactive(c(input$selectworkclass_1, input$yDataMin__3_, input$yDataMax__3_), { 
  if( ! is.na(input$yDataMin__3_) & ! is.na(input$yDataMax__3_)) {
    dfC() %>% dplyr::filter(between(hours_per_week, input$yDataMin__3_, input$yDataMax__3_))
  }
  else {
    dfC()
  }
