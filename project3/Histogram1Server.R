...
# Build and Output Throughput Plot ------------------------
  output$Histogram__1_ <- renderPlotly({
    plot = dfA1() %>% ggplot() + 
      geom_histogram(mapping = aes(x=age, colour = relationship, fill = relationship), binwidth = 10) +
      ...
      theme(plot.margin=unit(c(2,2,2,2,2,2),"cm")) +
      scale_y_continuous(labels = scales::comma) + # Disable scientific notation
      ggtitle(paste(Relationships(), input$title__1_)) +
